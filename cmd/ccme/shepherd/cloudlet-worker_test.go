	  ...
	  "alerts": [
		{
		  "labels": {
			...
			"` + edgeproto.ClusterKeyTagOrganization + `": "` + shepherd_test.TestClusterKey.Organization + `",
...
func TestCloudletAlerts(t *testing.T) {
	...
	cloudletAlertPruner := shepherd_common.NewCloudletAlertPruner(shepherd_test.TestCloudletKey)
	shepherd_common.UpdateAlertsCache(ctx, alerts, &AlertCache, cloudletAlertPruner)
	//should be no alerts
	...
	for _, alert := range AlertCache.Objs {
		...
		assert.Equal(t, shepherd_test.TestClusterKey.Organization, alert.Obj.Labels[edgeproto.ClusterKeyTagOrganization])
