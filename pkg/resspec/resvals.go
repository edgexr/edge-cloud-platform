...
func (s ResValMap) AddOptResMap(optResMap map[string]string) error {
	for resName, val := range optResMap {
		if err := s.AddOptRes(resName, val); err != nil {
			return err
		}
	}
	return nil
...
func (s ResValMap) AddOptRes(resName, val string) error {
	...
	nres := NewWholeResVal(name, "", uint64(count))
...
func (s ResValMap) AddNodeResources(nr *edgeproto.NodeResources) error {
	if nr == nil {
		return nil
	}
	s.AddVcpus(nr.Vcpus, 0)
	s.AddRam(nr.Ram)
	s.AddDisk(nr.Disk)
	// optional resources
	return s.AddOptResMap(nr.OptResMap)
}
...
func (s ResValMap) AddNodePoolResources(npr *edgeproto.NodePoolResources) error {
	...
	// optional resources
	return s.AddOptResMap(npr.TotalOptRes)
}
