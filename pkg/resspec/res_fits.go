...
func NodePoolFits(ctx context.Context, reqs *edgeproto.NodePoolResources, used ResValMap, nodePools []*edgeproto.NodePool, flavorLookup edgeproto.FlavorLookup) error {
	...
	for _, pool := range nodePools {
		...
		if !reqs.UsesGpu() && pool.NodeResources.UsesGpu() {
...
func NodeResourcesToResValMap(nr *edgeproto.NodeResources) (ResValMap, error) {
	resVals := ResValMap{}
	if err := resVals.AddNodeResources(nr); err != nil {
		return nil, err
	}
	return resVals, nil
}
...
func TopologyToResValMap(top *edgeproto.NodePoolTopology) (ResValMap, error) {
	...
	resVals.Add(NewWholeResVal(cloudcommon.ResourceVcpus, "", top.MinNodeCpus))
	...
	// optional resources
	err := resVals.AddOptResMap(top.MinNodeOptRes)
	if err != nil {
		return nil, err
	}
	return resVals, nil
