...
func (s *AppInstApi) UpdateAppInst(in *edgeproto.AppInst, cb edgeproto.AppInstApi_UpdateAppInstServer) error {
	...
	if fmap.HasOrHasChild(edgeproto.AppInstFieldKubernetesResources) {
		// TODO:
		// We should be able to update resource requirements,
		// but it is not implemented at this time.
		// We would need to check that the cluster supports the
		// new resource requirements, then commit the changes,
		// which effectively reserves those resources since the
		// AppInst is already referenced by the ClusterRefs.
		// For now we return an error.
		return errors.New("kubernetes resource update not supported yet")
	}
	resChange := false
...
func (s *AppInstApi) sumRequestedAppInstResources(appInst *edgeproto.AppInst) InfraResources {
	...
	if appInst.KubernetesResources != nil {
		res.AddNodePoolResources(appInst.KubernetesResources.CpuPool)
		res.AddNodePoolResources(appInst.KubernetesResources.GpuPool)
	}
	if appInst.NodeResources != nil {
		res.AddNodeRes(appInst.NodeResources, 1)
	}
	return res
}
...
func (s *AppInstApi) RecordAppInstEvent(ctx context.Context, appInst *edgeproto.AppInst, event cloudcommon.InstanceEvent, serverStatus string) {
	...
	resInfo := s.sumRequestedAppInstResources(appInst)
