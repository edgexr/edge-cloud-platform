...
func (s *ClusterInstApi) validateCloudletInfraResources(ctx context.Context, stm concurrency.STM, cloudlet *edgeproto.Cloudlet, cloudletInfo *edgeproto.CloudletInfo, infraResources *edgeproto.InfraResourcesSnapshot, usedResources, reqdResources, subtractReqdResources *CloudletResources) ([]string, error) {
	...
	if subtractReqdResources != nil {
		...
		reqdResInfo.SubFloor(subResInfo)
	}
	...
	for resName, res := range usedResInfo {
		...
		if resReq, ok := reqdResInfo[resName]; ok {
			resPrint.Req = resReq.Value
		}
...
func (s *ClusterInstApi) getCloudletResourceMetric(ctx context.Context, stm concurrency.STM, key *edgeproto.CloudletKey) ([]*edgeproto.Metric, error) {
	...
	ramUsed := resInfo.Get(cloudcommon.ResourceRamMb)
	vcpusUsed := resInfo.Get(cloudcommon.ResourceVcpus)
	gpusUsed := resInfo.Get(cloudcommon.ResourceGpus)
	externalIPsUsed := resInfo.Get(cloudcommon.ResourceExternalIPs)
...
func (s *ClusterInstApi) sumRequestedClusterResources(cluster *edgeproto.ClusterInst) InfraResources {
	...
	if cluster.NodeResources != nil {
		res.AddNodeRes(cluster.NodeResources, 1)
		numInsts++
	}
	for _, pool := range cluster.NodePools {
		res.AddNodePool(pool)
		numInsts += uint32(pool.NumNodes)
	}
	res.AddResValue(cloudcommon.ResourceInstances, uint64(numInsts))
	return res
}
...
func (s *ClusterInstApi) RecordClusterInstEvent(ctx context.Context, cluster *edgeproto.ClusterInst, event cloudcommon.InstanceEvent, serverStatus string) {
	...
	metric.AddIntVal(cloudcommon.MetricTagRAM, resInfo.Get(cloudcommon.ResourceRamMb))
	metric.AddIntVal(cloudcommon.MetricTagVCPU, resInfo.Get(cloudcommon.ResourceVcpus))
	metric.AddIntVal(cloudcommon.MetricTagDisk, resInfo.Get(cloudcommon.ResourceDiskGb))
	metric.AddIntVal(cloudcommon.MetricTagGPUs, resInfo.Get(cloudcommon.ResourceGpus))
	metric.AddIntVal(cloudcommon.MetricTagNodeCount, resInfo.Get(cloudcommon.ResourceInstances))
