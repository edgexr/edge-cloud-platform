...
// LocalTLSCertsDir contains certs generated by gen-test-certs.sh
...
func GetLocalTLSData() ([]byte, []byte, error) {
	if _, err := os.Stat(LocalTLSCertsDir + "/test-server.key"); err != nil {
		return nil, nil, fmt.Errorf("Key file %s/test-server.key not found, please ensure gen-test-certs.sh was run", LocalTLSCertsDir)
	}
	key, err := os.ReadFile(LocalTLSCertsDir + "/test-server.key")
	if err != nil {
		return nil, nil, err
	}
	cert, err := os.ReadFile(LocalTLSCertsDir + "/test-server.crt")
	if err != nil {
		return nil, nil, err
	}
	return key, cert, nil
