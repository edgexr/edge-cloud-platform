...
func (v *VMPlatform) GetClusterSubnetName(ctx context.Context, clusterInst *edgeproto.ClusterInst) SubnetNames {
	subnetName := MexSubnetPrefix + k8smgmt.GetCloudletClusterName(clusterInst)
...
func (v *VMPlatform) setupClusterRootLBAndNodes(ctx context.Context, rootLBName string, clusterInst *edgeproto.ClusterInst, updateCallback edgeproto.CacheUpdateCallback, start time.Time, timeout time.Duration, vmgp *VMGroupOrchestrationParams, action ActionType) (reterr error) {
	...
	if clusterInst.IpAccess == edgeproto.IpAccess_IP_ACCESS_DEDICATED {
		...
		err := v.SetupRootLB(ctx, rootLBName, rootLBName, &clusterInst.CloudletKey, &TrustPolicy, rootLBDetail, clusterInst.EnableIpv6, updateCallback)
		if err != nil {
			return err
		}
	}
...
func (v *VMPlatform) logEdgeCloudServeiceStatus(ctx context.Context, clusterInst *edgeproto.ClusterInst, masterName string, masterIPs ServerIPs, rootLBName string) {
	...
	if err != nil {
		log.SpanLog(ctx, log.DebugLevelInfra, "failed to get edgecloud status", "err", err)
		return
	}
	...
	if err != nil {
		log.SpanLog(ctx, log.DebugLevelInfra, "failed to get output of edgecloud", "err", err)
		return
	}
