...
type SendCloudletRefsHandler interface {
	GetAllLocked(ctx context.Context, cb func(key *edgeproto.CloudletRefs, modRev int64))
	GetWithRev(key *edgeproto.CloudletKey, buf *edgeproto.CloudletRefs, modRev *int64) bool
}

type RecvCloudletRefsHandler interface {
	Update(ctx context.Context, in *edgeproto.CloudletRefs, rev int64)
	Delete(ctx context.Context, in *edgeproto.CloudletRefs, rev int64)
	Prune(ctx context.Context, keys map[edgeproto.CloudletKey]struct{})
	Flush(ctx context.Context, notifyId int64)
}

type CloudletRefsCacheHandler interface {
	SendCloudletRefsHandler
	RecvCloudletRefsHandler
	AddNotifyCb(fn func(ctx context.Context, obj *edgeproto.CloudletRefs, modRev int64))
}

type CloudletRefsSend struct {
	Name        string
	MessageName string
	handler     SendCloudletRefsHandler
	Keys        map[edgeproto.CloudletKey]CloudletRefsSendContext
	keysToSend  map[edgeproto.CloudletKey]CloudletRefsSendContext
	notifyId    int64
	Mux         sync.Mutex
	buf         edgeproto.CloudletRefs
	SendCount   uint64
	sendrecv    *SendRecv
}

type CloudletRefsSendContext struct {
	ctx         context.Context
	modRev      int64
	forceDelete bool
}

func NewCloudletRefsSend(handler SendCloudletRefsHandler) *CloudletRefsSend {
	send := &CloudletRefsSend{}
	send.Name = "CloudletRefs"
	send.MessageName = proto.MessageName((*edgeproto.CloudletRefs)(nil))
	send.handler = handler
	send.Keys = make(map[edgeproto.CloudletKey]CloudletRefsSendContext)
	return send
}

func (s *CloudletRefsSend) SetSendRecv(sendrecv *SendRecv) {
	s.sendrecv = sendrecv
}

func (s *CloudletRefsSend) GetMessageName() string {
	return s.MessageName
}

func (s *CloudletRefsSend) GetName() string {
	return s.Name
}

func (s *CloudletRefsSend) GetSendCount() uint64 {
	return s.SendCount
}

func (s *CloudletRefsSend) GetNotifyId() int64 {
	return s.notifyId
}
func (s *CloudletRefsSend) UpdateAll(ctx context.Context) {
	if !s.sendrecv.isRemoteWanted(s.MessageName) {
		return
	}
	s.Mux.Lock()
	s.handler.GetAllLocked(ctx, func(obj *edgeproto.CloudletRefs, modRev int64) {
		s.Keys[*obj.GetKey()] = CloudletRefsSendContext{
			ctx:    ctx,
			modRev: modRev,
		}
	})
...
func (s *CloudletRefsSend) Update(ctx context.Context, obj *edgeproto.CloudletRefs, modRev int64) {
	if !s.sendrecv.isRemoteWanted(s.MessageName) {
		return
	}
	forceDelete := false
...
func (s *CloudletRefsSend) ForceDelete(ctx context.Context, key *edgeproto.CloudletKey, modRev int64) {
	forceDelete := true
	s.updateInternal(ctx, key, modRev, forceDelete)
}

func (s *CloudletRefsSend) updateInternal(ctx context.Context, key *edgeproto.CloudletKey, modRev int64, forceDelete bool) {
	s.Mux.Lock()
	log.SpanLog(ctx, log.DebugLevelNotify, "updateInternal CloudletRefs", "key", key, "modRev", modRev)
	s.Keys[*key] = CloudletRefsSendContext{
		ctx:         ctx,
		modRev:      modRev,
...
func (s *CloudletRefsSend) SendForCloudlet(ctx context.Context, action edgeproto.NoticeAction, cloudlet *edgeproto.Cloudlet) {
}

func (s *CloudletRefsSend) Send(stream StreamNotify, notice *edgeproto.Notice, peer string) error {
	s.Mux.Lock()
	keys := s.keysToSend
	s.keysToSend = nil
	s.Mux.Unlock()
	for key, sendContext := range keys {
		...
		log.SpanLog(ctx, log.DebugLevelNotify,
			fmt.Sprintf("%s send CloudletRefs", s.sendrecv.cliserv),
			"peerAddr", peer,
...
func (s *CloudletRefsSend) PrepData() bool {
	s.Mux.Lock()
	defer s.Mux.Unlock()
	if len(s.Keys) > 0 {
		s.keysToSend = s.Keys
		s.Keys = make(map[edgeproto.CloudletKey]CloudletRefsSendContext)
		return true
	}
	return false
}
...
// peers to send to.
type CloudletRefsSendMany struct {
	handler SendCloudletRefsHandler
	Mux     sync.Mutex
	sends   map[string]*CloudletRefsSend
}

func NewCloudletRefsSendMany(handler SendCloudletRefsHandler) *CloudletRefsSendMany {
	s := &CloudletRefsSendMany{}
	s.handler = handler
	s.sends = make(map[string]*CloudletRefsSend)
	return s
}

func (s *CloudletRefsSendMany) NewSend(peerAddr string, notifyId int64) NotifySend {
	send := NewCloudletRefsSend(s.handler)
...
func (s *CloudletRefsSendMany) DoneSend(peerAddr string, send NotifySend) {
	asend, ok := send.(*CloudletRefsSend)
	if !ok {
		return
	}
...
func (s *CloudletRefsSendMany) Update(ctx context.Context, obj *edgeproto.CloudletRefs, modRev int64) {
	s.Mux.Lock()
	defer s.Mux.Unlock()
...
func (s *CloudletRefsSendMany) GetTypeString() string {
	return "CloudletRefs"
}

type CloudletRefsRecv struct {
	Name        string
	MessageName string
	handler     RecvCloudletRefsHandler
...
func NewCloudletRefsRecv(handler RecvCloudletRefsHandler) *CloudletRefsRecv {
	recv := &CloudletRefsRecv{}
	recv.Name = "CloudletRefs"
	recv.MessageName = proto.MessageName((*edgeproto.CloudletRefs)(nil))
	recv.handler = handler
	return recv
}

func (s *CloudletRefsRecv) SetSendRecv(sendrecv *SendRecv) {
	s.sendrecv = sendrecv
}

func (s *CloudletRefsRecv) GetMessageName() string {
	return s.MessageName
}

func (s *CloudletRefsRecv) GetName() string {
	return s.Name
}

func (s *CloudletRefsRecv) GetRecvCount() uint64 {
	return s.RecvCount
}

func (s *CloudletRefsRecv) Recv(ctx context.Context, notice *edgeproto.Notice, notifyId int64, peerAddr string) {
	span := opentracing.SpanFromContext(ctx)
	if span != nil {
		span.SetTag("objtype", "CloudletRefs")
	}
	...
	log.SpanLog(ctx, log.DebugLevelNotify,
		fmt.Sprintf("%s recv CloudletRefs", s.sendrecv.cliserv),
		"peerAddr", peerAddr,
		"peer", s.sendrecv.peer,
		"local", s.sendrecv.name,
		"action", notice.Action,
...
func (s *CloudletRefsRecv) RecvAllStart() {
	s.Mux.Lock()
	defer s.Mux.Unlock()
	s.sendAllKeys = make(map[edgeproto.CloudletKey]struct{})
}

func (s *CloudletRefsRecv) RecvAllEnd(ctx context.Context, cleanup Cleanup) {
	s.Mux.Lock()
...
func (s *CloudletRefsRecv) Flush(ctx context.Context, notifyId int64) {
	s.handler.Flush(ctx, notifyId)
}

type CloudletRefsRecvMany struct {
	handler RecvCloudletRefsHandler
}

func NewCloudletRefsRecvMany(handler RecvCloudletRefsHandler) *CloudletRefsRecvMany {
	s := &CloudletRefsRecvMany{}
	s.handler = handler
	return s
}

func (s *CloudletRefsRecvMany) NewRecv() NotifyRecv {
	recv := NewCloudletRefsRecv(s.handler)
	return recv
}

func (s *CloudletRefsRecvMany) Flush(ctx context.Context, notifyId int64) {
	s.handler.Flush(ctx, notifyId)
}
func (mgr *ServerMgr) RegisterSendCloudletRefsCache(cache CloudletRefsCacheHandler) {
	send := NewCloudletRefsSendMany(cache)
	mgr.RegisterSend(send)
	cache.AddNotifyCb(send.Update)
}

func (mgr *ServerMgr) RegisterRecvCloudletRefsCache(cache CloudletRefsCacheHandler) {
	recv := NewCloudletRefsRecvMany(cache)
	mgr.RegisterRecv(recv)
}

func (s *Client) RegisterSendCloudletRefsCache(cache CloudletRefsCacheHandler) {
	send := NewCloudletRefsSend(cache)
	s.RegisterSend(send)
	cache.AddNotifyCb(send.Update)
}

func (s *Client) RegisterRecvCloudletRefsCache(cache CloudletRefsCacheHandler) {
	recv := NewCloudletRefsRecv(cache)
	s.RegisterRecv(recv)
}
