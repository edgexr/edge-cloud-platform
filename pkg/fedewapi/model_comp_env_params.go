...
var CompEnvParamsEnvVarNamePattern = strings.TrimPrefix(strings.TrimSuffix("/^[A-Za-z0-9][A-Za-z0-9_]{0,63}$/", "/"), "/")
var CompEnvParamsEnvVarNameRE = regexp.MustCompile(CompEnvParamsEnvVarNamePattern)
var CompEnvParamsEnvVarValuePattern = strings.TrimPrefix(strings.TrimSuffix("/^[A-Za-z0-9_,\\.-]{0,128}$/", "/"), "/")
var CompEnvParamsEnvVarValueRE = regexp.MustCompile(CompEnvParamsEnvVarValuePattern)

func (s *CompEnvParams) Validate() error {
	if s.EnvVarName == "" {
		return errors.New("envVarName is required")
	}
	if !CompEnvParamsEnvVarNameRE.MatchString(s.EnvVarName) {
		return errors.New("envVarName " + s.EnvVarName + " does not match format " + CompEnvParamsEnvVarNamePattern)
	}
	if s.EnvValueType == "" {
