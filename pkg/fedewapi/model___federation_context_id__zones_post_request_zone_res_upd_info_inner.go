...
func (s *FederationContextIdZonesPostRequestZoneResUpdInfoInner) Validate() error {
	for ii := range s.AvailableCompResources {
		if err := s.AvailableCompResources[ii].Validate(); err != nil {
			return err
		}
	}
	if s.AvailableNetResources != nil {
		if err := s.AvailableNetResources.Validate(); err != nil {
			return err
		}
	}
	return nil
