...
func (s *ViewISVResPool200ResponseInner) Validate() error {
	...
	if !ViewISVResPool200ResponseInnerReservedPoolIdRE.MatchString(s.ReservedPoolId) {
		return errors.New("reservedPoolId " + s.ReservedPoolId + " does not match format " + ViewISVResPool200ResponseInnerReservedPoolIdPattern)
	}
	for ii := range s.ReservedFlavours {
		if err := s.ReservedFlavours[ii].Validate(); err != nil {
			return err
		}
	}
	if s.ReserveDuration != nil {
