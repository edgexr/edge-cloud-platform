...
func (s *FederationRequestData) Validate() error {
	if s.OrigOPFederationId == "" {
		return errors.New("origOPFederationId is required")
	}
	if !FederationRequestDataOrigOPFederationIdRE.MatchString(s.OrigOPFederationId) {
		return errors.New("origOPFederationId " + s.OrigOPFederationId + " does not match format " + FederationRequestDataOrigOPFederationIdPattern)
	}
	if s.OrigOPCountryCode != nil && !FederationRequestDataOrigOPCountryCodeRE.MatchString(*s.OrigOPCountryCode) {
		return errors.New("origOPCountryCode " + *s.OrigOPCountryCode + " does not match format " + FederationRequestDataOrigOPCountryCodePattern)
	}
	if s.OrigOPMobileNetworkCodes != nil {
		if err := s.OrigOPMobileNetworkCodes.Validate(); err != nil {
			return err
		}
	}
	if s.PartnerStatusLink == "" {
		return errors.New("partnerStatusLink is required")
	}
	if s.PartnerCallbackCredentials != nil {
		if err := s.PartnerCallbackCredentials.Validate(); err != nil {
			return err
		}
	}
	return nil
