...
func (s *InstallAppRequestZoneInfo) Validate() error {
	...
	if !InstallAppRequestZoneInfoZoneIdRE.MatchString(s.ZoneId) {
		return errors.New("zoneId " + s.ZoneId + " does not match format " + InstallAppRequestZoneInfoZoneIdPattern)
	}
	if s.FlavourId == "" {
		return errors.New("flavourId is required")
	}
	if s.ResPool != nil && !InstallAppRequestZoneInfoResPoolRE.MatchString(*s.ResPool) {
		return errors.New("resPool " + *s.ResPool + " does not match format " + InstallAppRequestZoneInfoResPoolPattern)
	}
	return nil
}
