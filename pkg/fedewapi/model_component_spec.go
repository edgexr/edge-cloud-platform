...
func (s *ComponentSpec) Validate() error {
	...
	if !ComponentSpecComponentNameRE.MatchString(s.ComponentName) {
		return errors.New("componentName " + s.ComponentName + " does not match format " + ComponentSpecComponentNamePattern)
	}
	if s.RestartPolicy == "" {
	...
	for ii := range s.ExposedInterfaces {
		if err := s.ExposedInterfaces[ii].Validate(); err != nil {
			return err
		}
	}
	if err := s.ComputeResourceProfile.Validate(); err != nil {
		return err
	}
	for ii := range s.CompEnvParams {
		if err := s.CompEnvParams[ii].Validate(); err != nil {
			return err
		}
	}
	...
	for ii := range s.PersistentVolumes {
		if err := s.PersistentVolumes[ii].Validate(); err != nil {
			return err
		}
	}
	return nil
