...
func (s *FederationContextIdZonesPostRequest) Validate() error {
	...
	if !FederationContextIdZonesPostRequestFederationContextIdRE.MatchString(s.FederationContextId) {
		return errors.New("federationContextId " + s.FederationContextId + " does not match format " + FederationContextIdZonesPostRequestFederationContextIdPattern)
	}
	if s.ZoneId == "" {
		return errors.New("zoneId is required")
	}
	if !FederationContextIdZonesPostRequestZoneIdRE.MatchString(s.ZoneId) {
		return errors.New("zoneId " + s.ZoneId + " does not match format " + FederationContextIdZonesPostRequestZoneIdPattern)
	}
	for ii := range s.ZoneResUpdInfo {
		if err := s.ZoneResUpdInfo[ii].Validate(); err != nil {
			return err
		}
	}
	return nil
