...
func (s *AppComponentSpecsInner) Validate() error {
	if s.ServiceNameNB != nil && !AppComponentSpecsInnerServiceNameNBRE.MatchString(*s.ServiceNameNB) {
		return errors.New("serviceNameNB " + *s.ServiceNameNB + " does not match format " + AppComponentSpecsInnerServiceNameNBPattern)
	}
	if s.ServiceNameEW != nil && !AppComponentSpecsInnerServiceNameEWRE.MatchString(*s.ServiceNameEW) {
		return errors.New("serviceNameEW " + *s.ServiceNameEW + " does not match format " + AppComponentSpecsInnerServiceNameEWPattern)
	}
	if s.ComponentName != nil && !AppComponentSpecsInnerComponentNameRE.MatchString(*s.ComponentName) {
		return errors.New("componentName " + *s.ComponentName + " does not match format " + AppComponentSpecsInnerComponentNamePattern)
	}
	if s.ArtefactId == "" {
		return errors.New("artefactId is required")
	}
	if !AppComponentSpecsInnerArtefactIdRE.MatchString(s.ArtefactId) {
		return errors.New("artefactId " + s.ArtefactId + " does not match format " + AppComponentSpecsInnerArtefactIdPattern)
	}
	return nil
}
