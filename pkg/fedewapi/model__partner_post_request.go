...
func (s *PartnerPostRequest) Validate() error {
	if s.FederationContextId == "" {
		return errors.New("federationContextId is required")
	}
	if !PartnerPostRequestFederationContextIdRE.MatchString(s.FederationContextId) {
		return errors.New("federationContextId " + s.FederationContextId + " does not match format " + PartnerPostRequestFederationContextIdPattern)
	}
	if s.ObjectType == "" {
	...
	if s.LcmSvcEndPoint != nil {
		if err := s.LcmSvcEndPoint.Validate(); err != nil {
			return err
		}
	}
	...
	if s.RemoveMobileNetworkIds != nil {
		if err := s.RemoveMobileNetworkIds.Validate(); err != nil {
			return err
		}
	}
	for ii := range s.AddZones {
		if err := s.AddZones[ii].Validate(); err != nil {
			return err
		}
	}
	for ii := range s.RemoveZones {
		if !PartnerPostRequestRemoveZonesRE.MatchString(s.RemoveZones[ii]) {
			return errors.New("RemoveZones " + s.RemoveZones[ii] + " does not match format " + PartnerPostRequestRemoveZonesPattern)
		}
	}
	for ii := range s.ZoneStatus {
		if err := s.ZoneStatus[ii].Validate(); err != nil {
			return err
		}
	}
	return nil
