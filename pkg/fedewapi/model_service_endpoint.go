...
var ServiceEndpointIpv4AddressesPattern = strings.TrimPrefix(strings.TrimSuffix("", "/"), "/")
var ServiceEndpointIpv4AddressesRE = regexp.MustCompile(ServiceEndpointIpv4AddressesPattern)

func (s *ServiceEndpoint) Validate() error {
	for ii := range s.Ipv4Addresses {
		if !ServiceEndpointIpv4AddressesRE.MatchString(s.Ipv4Addresses[ii]) {
			return errors.New("Ipv4Addresses " + s.Ipv4Addresses[ii] + " does not match format " + ServiceEndpointIpv4AddressesPattern)
		}
	}
	return nil
}
