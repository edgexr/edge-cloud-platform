...
func testDummyRedisServer(t *testing.T, client *redis.Client, server *DummyRedis) {
	...
	err := IsServerReady(ctx, client, 5*time.Second)
	require.Nil(t, err)

	resStr, err := client.Set(ctx, "k1", "v1", 0).Result()
	require.Nil(t, err)
	require.Equal(t, "OK", resStr, "set key without any expiry time")

	resStr, err = client.Set(ctx, "k1", "v1", 100*time.Millisecond).Result()
	require.Nil(t, err)
	require.Equal(t, "OK", resStr, "update key with expiry time")

	resStr, err = client.Get(ctx, "k1").Result()
	require.Nil(t, err)
	...
	resBool, err := client.SetNX(ctx, "k2", "v2", 0).Result()
	require.Nil(t, err)
	require.True(t, resBool, "set key which will never expire")

	resBool, err = client.SetNX(ctx, "k2", "v2", 100*time.Millisecond).Result()
	require.Nil(t, err)
	...
	for _, msg := range msgs1 {
		_, err = client.Publish(ctx, "ch1", msg).Result()
		require.Nil(t, err)
	}
	msgs2 := []string{"msg4", "msg5", "msg6"}
	for _, msg := range msgs2 {
		_, err = client.Publish(ctx, "ch2", msg).Result()
		require.Nil(t, err)
	}
	...
	keysRem, err := client.Del(ctx, "k1", "k2").Result()
	require.Nil(t, err)
	...
	resStr, err = client.Get(ctx, "k2").Result()
	require.Equal(t, redis.Nil, err, "key should not exist")
	...
	resStr, err = client.Set(ctx, nonstreamKey, "v1", 0).Result()
	require.Nil(t, err)
	require.Equal(t, "OK", resStr, "set key without any expiry time")

	out, err := client.XRange(ctx, streamKey1, RedisSmallestId, RedisGreatestId).Result()
	require.Nil(t, err)
	require.Equal(t, 0, len(out))
	out, err = client.XRange(ctx, streamKey2, RedisSmallestId, RedisGreatestId).Result()
	require.Nil(t, err)
	require.Equal(t, 0, len(out))
	out, err = client.XRange(ctx, nonstreamKey, RedisSmallestId, RedisGreatestId).Result()
	...
	_, err = client.XAdd(ctx, &xaddArgs).Result()
	require.Nil(t, err)
	...
	out, err = client.XRange(ctx, streamKey1, "invalidID", RedisGreatestId).Result()
	require.NotNil(t, err)
	require.Equal(t, "ERR Invalid stream ID specified as stream command argument", err.Error())

	out, err = client.XRange(ctx, streamKey1, RedisSmallestId, "invalidID").Result()
	require.NotNil(t, err)
	require.Equal(t, "ERR Invalid stream ID specified as stream command argument", err.Error())
	...
	out, err = client.XRange(ctx, streamKey1, lastStream1ID, RedisGreatestId).Result()
	require.Nil(t, err)
	require.Equal(t, 2, len(out))
	require.NotEmpty(t, out[1].ID, "")
	require.Equal(t, out[1].Values, streamMsg2)
	newStream1ID := out[1].ID
	out, err = client.XRange(ctx, streamKey2, lastStream2ID, RedisGreatestId).Result()
	require.Nil(t, err)
	...
	xreadOut, err := client.XRead(ctx, &xreadArgs).Result()
	require.NotNil(t, err)
	require.Equal(t, "ERR Invalid stream ID specified as stream command argument", err.Error())
	...
	// cleanup
	keysRem, err = client.Del(ctx, nonstreamKey, streamKey1, streamKey2).Result()
	require.Nil(t, err)
