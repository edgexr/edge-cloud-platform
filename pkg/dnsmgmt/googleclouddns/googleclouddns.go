...
const vaultPath = "/secret/data/accounts/googleclouddns"
...
func GetProvider(ctx context.Context, vaultConfig *vault.Config) (dnsapi.Provider, error) {
	...
	err := vault.GetData(vaultConfig, vaultPath, 0, &data)
	if err != nil {
		return nil, err
	}
	return getProvider(ctx, data)
}

func getProvider(ctx context.Context, jsonCreds map[string]string) (dnsapi.Provider, error) {
	project, ok := jsonCreds[projectID]
	if !ok {
		return nil, fmt.Errorf("google cloud DNS credentials missing " + projectID)
	}
	jsonData, err := json.Marshal(jsonCreds)
	if err != nil {
		return nil, err
	}
