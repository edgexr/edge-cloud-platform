...
func MergeEnvVars(ctx context.Context, accessApi platform.AccessApi, app *edgeproto.App, appInst *edgeproto.AppInst, mf string, imagePullSecrets []string, names *KubeNames, kr *edgeproto.KubernetesResources) (string, error) {
	...
	for i, _ := range objs {
		...
		gpuCountsByMaker := map[string]int{}
		if kr.GpuPool != nil {
			for _, gpu := range kr.GpuPool.TotalGpus {
				gpuCountsByMaker[gpu.Vendor]++
			}
			...
			if optResGpuCount := cloudcommon.GetOptResGPUCount(kr.GpuPool.TotalOptRes); optResGpuCount > 0 {
				...
				// nvidia gpus.
				gpuCountsByMaker[cloudcommon.GPUVendorNVIDIA] += int(optResGpuCount)
			}
		}
		if len(gpuCountsByMaker) > 0 {
			...
			for maker, gpuCount := range gpuCountsByMaker {
				gpuResName := GetResourceNameForGpuMaker(maker)
				if gpuResName == "" {
					return "", fmt.Errorf("no Kubernetes resource name defined for gpu maker %s", maker)
				}
...
func GetResourceNameForGpuMaker(gpuMaker string) string {
	switch gpuMaker {
