// Copyright 2022 MobiledgeX, Inc
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//      http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package vault

import (
	"errors"
	"fmt"
	"io"
	"net/http"
	"sync"
	"time"

	"github.com/MicahParks/jwkset"
	"github.com/MicahParks/keyfunc/v3"
	"github.com/edgexr/edge-cloud-platform/pkg/log"
	jwt "github.com/golang-jwt/jwt/v5"
	"github.com/mitchellh/mapstructure"
)

// JWKS is a set of JWT keys to support rotating keys.
// Multiple keys are stored, indexed by Key ID.
// The Key ID is set on the JWT claims, so it can be looked up
// during verification.
// Rotation works by another process adding a new version of the
// secret to Vault. Vault maintains the older values of the secret,
// where we use the vault secret version as the key ID.
// If VerifyCookie encounters a Key ID greater than it's current
// one (implying another process had updated to a newer version),
// then it will attempt to update inline. Older key versions are
// handled by just looking up the older version. The refresh
// interval should be much smaller than the rotation interval
// to allow for all instances to update to the next rotation
// before the secret is rotated again.
// This implementation does not support the full JWKS spec feature
// set, for example being able to specify the signing algorithm, etc
// in the claims.

// JWKS stores the multiple versions of data retrieved from Vault,
// as well as the data needed to access Vault.
type JWKS struct {
	Keys                map[int]*JWK
	Meta                KVMetadata
	RefreshDelay        time.Duration
	Mux                 sync.Mutex
	Path                string
	Metapath            string
	config              Config
	wellKnownURL        string
	wellKnownKeys       keyfunc.Keyfunc
	lastUpdateAttempt   time.Time
	lastWellKnownUpdate time.Time
}

// JWK is the data stored in Vault
type JWK struct {
	Secret  string
	Refresh string
}

var DefaultJwkRefreshDelay = 5 * time.Minute
var JwkUpdateDelay = 5 * time.Second
var WellKnownUpdateDelay = 15 * time.Minute

func (s *JWKS) Init(config *Config, region, name string, op ...JWKOp) {
	opts := JWKOptions{}
	for _, o := range op {
		o(&opts)
	}
	if region != "" {
		region += "/"
	}
	s.Keys = make(map[int]*JWK)
	s.RefreshDelay = DefaultJwkRefreshDelay
	s.Path = region + "jwtkeys/data/" + name
	s.Metapath = region + "jwtkeys/metadata/" + name
	s.config = *config
	s.wellKnownURL = opts.wellKnownURL
	s.lastWellKnownUpdate = time.Unix(0, 0)
	s.lastUpdateAttempt = time.Unix(0, 0)
}

// GoUpdate starts a go thread to keep the JKWS up to date.
// A chan struct can be passed in which will be closed once
// the first iteration is done and the key set was downloaded
// from Vault.
func (s *JWKS) GoUpdate(callerDone chan struct{}, updateDone chan struct{}) {
	// make initial attempt immediately
	refreshDelay := time.Duration(0)
	go func() {
		for {
			select {
			case <-callerDone:
				return
			case <-time.After(refreshDelay):
				refreshDelay = s.RefreshDelay
			}

			err := s.UpdateKeys()
			if updateDone != nil {
				close(updateDone)
				updateDone = nil
			}
			if err != nil {
				log.InfoLog("jwks update keys", "path", s.Path, "metapath", s.Metapath, "err", err)
				continue
			}
		}
	}()
}

func (s *JWKS) GetCurrentKey() (string, int, bool) {
	version := s.Meta.CurrentVersion
	str, ok := s.GetKey(version)
	return str, version, ok
}

func (s *JWKS) GetKey(version int) (string, bool) {
	if version > s.Meta.CurrentVersion {
		// New secret may have been pushed to Vault and used
		// by another instance. See if we can grab it as well.
		err := s.UpdateKeys()
		if err != nil {
			return "", false
		}
	}
	s.Mux.Lock()
	defer s.Mux.Unlock()
	jwk, ok := s.Keys[version]
	if !ok {
		return "", false
	}
	return jwk.Secret, true
}

// For testing or debug only
func (s *JWKS) SetLastUpdateAttempt(t time.Time) {
	s.Mux.Lock()
	defer s.Mux.Unlock()
	s.lastUpdateAttempt = t
}

// KVJWK represents the kv data in vault returned by a specific version request
type KVJWK struct {
	Meta KVMeta `mapstructure:"metadata"`
	Data JWK    `mapstructure:"data"`
}

func (s *JWKS) UpdateKeys() error {
	s.Mux.Lock()
	if time.Since(s.lastUpdateAttempt) < JwkUpdateDelay {
		// avoid constant attempts
		s.Mux.Unlock()
		return fmt.Errorf("too many update attempts")
	}
	s.lastUpdateAttempt = time.Now()
	s.Mux.Unlock()

	client, err := s.config.Login()
	if err != nil {
		return fmt.Errorf("login failed for %s, %s", s.config.Addr, err.Error())
	}

	// Get the metadata to find out how many and what versions there are
	kvdata, err := GetKV(client, s.Metapath, -1)
	if err != nil {
		return err
	}
	metadata, err := ParseMetadata(kvdata)
	if err != nil {
		return err
	}

	// Key ID is set to the key version in Vault. This version is
	// automatically incremented whenever the key is updated, so it
	// is a natural JWT key ID.
	keys := make(map[int]*JWK)
	for ii := metadata.CurrentVersion; ii >= metadata.OldestVersion; ii-- {
		if _, found := metadata.Versions[ii]; !found {
			continue
		}
		kvdata, err := GetKV(client, s.Path, ii)
		if err != nil {
			return err
		}
		kvjwk := &KVJWK{}
		err = mapstructure.WeakDecode(kvdata, kvjwk)
		if err != nil {
			return err
		}
		if kvjwk.Meta.Version != ii {
			log.InfoLog("got JWT key version different from requested version, likely requested version not available or deleted", "path", s.Path, "requested", ii, "received", kvjwk.Meta.Version)
			continue
		}
		keys[kvjwk.Meta.Version] = &kvjwk.Data

		if ii == metadata.CurrentVersion && kvjwk.Data.Refresh != "" {
			dur, err := time.ParseDuration(kvjwk.Data.Refresh)
			if err != nil {
				log.InfoLog("parse refresh failed", "refresh",
					kvjwk.Data.Refresh, "err", err)
			} else {
				s.RefreshDelay = dur
			}
		}
	}

	// swap in new keys
	s.Mux.Lock()
	s.Meta = *metadata
	s.Keys = keys
	s.Mux.Unlock()
	return nil
}

func (s *JWKS) UpdateWellKnownKeys() error {
	// standard oauth2 jwks at well known URL
	if s.wellKnownURL == "" {
		log.DebugLog(log.DebugLevelApi, "update well known keys: no well-known URL configured")
		return nil
	}
	resp, err := http.Get(s.wellKnownURL)
	if err != nil {
		return fmt.Errorf("failed to fetch well-known keys from %s: %v", s.wellKnownURL, err)
	}
	defer resp.Body.Close()
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return fmt.Errorf("failed to read response from %s: %v", s.wellKnownURL, err)
	}
	keyset, err := keyfunc.NewJWKSetJSON(body)
	if err != nil {
		return fmt.Errorf("failed to parse well-known keys from %s: %v", s.wellKnownURL, err)
	}
	// swap in new keys
	s.Mux.Lock()
	s.wellKnownKeys = keyset
	s.lastWellKnownUpdate = time.Now()
	s.Mux.Unlock()
	return nil
}

func PutSecret(config *Config, region, name, secret, refresh string) error {
	client, err := config.Login()
	if err != nil {
		return err
	}
	if region != "" {
		region += "/"
	}
	path := region + "jwtkeys/data/" + name
	data := map[string]interface{}{
		"secret":  secret,
		"refresh": refresh,
	}
	out := map[string]interface{}{
		"data": data,
	}
	return PutKV(client, path, out)
}

type Claims interface {
	jwt.Claims
	// Get the Key ID with the claims (for VerifyCookie)
	GetKid() (int, error)
	// Set the Key ID with the claims (for GenerateCookie)
	SetKid(int)
	// Check if JWT was generated by us
	GetSourceMarker() string
	// Set if JWT was generated by us
	SetSourceMarker(string)
}

type SourceClaims struct {
	// Mark as generated by us
	SourceMarker string `json:"sourcemarker"`
}

func (s *SourceClaims) GetSourceMarker() string {
	return s.SourceMarker
}

func (s *SourceClaims) SetSourceMarker(v string) {
	s.SourceMarker = v
}

const (
	SourceMarkerInternal = "internal"
	// no way to mark externally generated JWTs
)

var ErrSourceMarker = errors.New("source marker mismatch")

func (s *JWKS) GenerateCookie(claims Claims) (string, error) {
	skey, kid, ok := s.GetCurrentKey()
	if !ok {
		return "", errors.New("no signing key")
	}
	claims.SetKid(kid)
	claims.SetSourceMarker(SourceMarkerInternal)
	token := jwt.NewWithClaims(jwt.SigningMethodHS512, claims)
	cookie, err := token.SignedString([]byte(skey))
	return cookie, err
}

func (s *JWKS) VerifyCookie(cookie string, claims Claims) (*jwt.Token, error) {
	if cookie == "" {
		return nil, errors.New("VerifyCookie failed: missing cookie")
	}
	return jwt.ParseWithClaims(cookie, claims, func(token *jwt.Token) (verifykey any, err error) {
		if claims.GetSourceMarker() != SourceMarkerInternal {
			return nil, fmt.Errorf("%w, expected internal", ErrSourceMarker)
		}
		kid, err := claims.GetKid()
		if err != nil {
			return nil, fmt.Errorf("VerifyCookie failed: %v", err)
		}
		secret, ok := s.GetKey(kid)
		if !ok {
			return nil, fmt.Errorf("VerifyCookie failed: no secret for kid %d in cookie %s", kid, cookie)
		}
		return []byte(secret), nil
	})
}

func (s *JWKS) VerifyOauth2Token(tokenString string, claims Claims) (*jwt.Token, error) {
	if tokenString == "" {
		return nil, errors.New("VerifyOauth2Token failed: missing token")
	}
	// if no keys or missing key, retry after fetching keys
	// this avoids a race condition at hydra startup where not
	// all keys are available even though hydra reports as "ready".
	for ii := range 2 {
		s.Mux.Lock()
		keyset := s.wellKnownKeys
		lastUpdate := s.lastWellKnownUpdate
		s.Mux.Unlock()

		token, err := jwt.ParseWithClaims(tokenString, claims, func(token *jwt.Token) (any, error) {
			if claims.GetSourceMarker() == SourceMarkerInternal {
				return nil, fmt.Errorf("%w, is internal", ErrSourceMarker)
			}
			if keyset == nil {
				// trigger key fetch
				return nil, fmt.Errorf("%w: no well-known keys", jwkset.ErrKeyNotFound)
			}
			return keyset.Keyfunc(token)
		})
		if err != nil && errors.Is(err, jwkset.ErrKeyNotFound) && time.Since(lastUpdate) > WellKnownUpdateDelay && ii == 0 {
			// retry with new keys
			s.UpdateWellKnownKeys()
			continue
		}
		return token, err
	}
	// should never reach here
	return nil, fmt.Errorf("VerifyOauth2Token failed: unreachable")
}

type JWKOptions struct {
	wellKnownURL string // url to well known jwk public keys
}

type JWKOp func(*JWKOptions)

func WithWellKnownURL(url string) JWKOp {
	return func(o *JWKOptions) {
		o.wellKnownURL = url
	}
}
