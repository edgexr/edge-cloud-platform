...
func TestJwk(t *testing.T) {
	...
	vroles, err := vaultProc.StartLocalRoles()
	require.Nil(t, err, "start local vault")
	defer vaultProc.StopLocal()

	roles := vroles.RegionRoles["local"]

	// this represents a dme process accessing vault
	vaultAddr := vaultProc.ListenAddr
	dmeAuth := vault.NewAppRoleAuth(roles.DmeRoleID, roles.DmeSecretID)
	dmeConfig := vault.NewConfig(vaultAddr, dmeAuth)
	jwks := vault.JWKS{}
	...
	rotatorAuth := vault.NewAppRoleAuth(roles.RotatorRoleID, roles.RotatorSecretID)
	rotatorConfig := vault.NewConfig(vaultAddr, rotatorAuth)
	newSecret := "abcdefg"
	err = vault.PutSecret(rotatorConfig, "local", "dme", newSecret, "1m")
	...
	jwks.SetLastUpdateAttempt(time.Now().Add(-time.Minute))
