...
func (x *ShowAutoScalePolicy) AssertFound(t *testing.T, obj *edgeproto.AutoScalePolicy) {
	...
	if found && !check.Matches(obj, edgeproto.MatchIgnoreBackend(), edgeproto.MatchSortArrayedKeys()) {
		...
		for _, field := range diffFields.Fields() {
			if _, found := edgeproto.AutoScalePolicyBackendFieldsMap[field]; found {
				continue
			}
			if _, found := edgeproto.AutoScalePolicyNoConfigFieldsMap[field]; found {
				continue
			}
			str := edgeproto.AutoScalePolicyAllFieldsStringMap[field]
			diffFieldStrs += str + ", "
		}
		require.Equal(t, *obj, check, "AutoScalePolicy differ in fields %v", diffFieldStrs)
	}
	if found {
