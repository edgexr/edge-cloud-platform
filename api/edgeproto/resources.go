...
// We categorize node resources and nodepools into different types.
// For resource allocation/allotment calculations, we want to be
// able to assume that nodepools with specialized resources only run
// workloads that require those specialized resources, otherwise
// workloads run on the general "cpu" pool. How this is enforced
// is left up to the Kubernetes scheduler and potentially
// additional affinity rules.

func (s *NodeResources) UsesGpu() bool {
	if s.OptResMap != nil && s.OptResMap["gpu"] != "" {
		return true
	}
	return false
}

func (s *NodePool) IsGpuPool() bool {
	if s.NodeResources != nil {
		return s.NodeResources.UsesGpu()
	}
	return false
}

func (s *NodePoolResources) UsesGpu() bool {
	_, found := s.TotalOptRes["gpu"]
	return found
}
...
func (s *KubernetesResources) SetFromFlavor(flavor *Flavor) {
	...
	s.CpuPool.Topology.MinNodeCpus = flavor.Vcpus
...
func (s *ServerlessConfig) ToKubernetesResources() *KubernetesResources {
	...
	return &KubernetesResources{
		CpuPool: &NodePoolResources{
			...
			Topology: NodePoolTopology{
				MinNodeCpus:      uint64(s.Vcpus.Whole),
				MinNodeMemory:    s.Ram,
...
func (s *AppInst) UsesGPU() bool {
	if s.KubernetesResources != nil && s.KubernetesResources.GpuPool != nil {
		return true
	}
	if s.NodeResources != nil {
		return s.NodeResources.UsesGpu()
