...
type AppInstanceInfo struct {
	...
	AppProvider AppProvider `json:"appProvider"`
	...
	ComponentEndpointInfo *[]AppInstanceInfo_ComponentEndpointInfo `json:"componentEndpointInfo,omitempty"`
	...
	// Name Name of the App instance, scoped to the AppProvider
	Name AppInstanceName `json:"name"`

	// Status Status of the application instance (default is 'unknown')
	Status *AppInstanceInfoStatus `json:"status,omitempty"`
}

// AppInstanceInfo_ComponentEndpointInfo defines model for AppInstanceInfo.ComponentEndpointInfo.
type AppInstanceInfo_ComponentEndpointInfo struct {
...
// AppInstanceName Name of the App instance, scoped to the AppProvider
type AppInstanceName = string
...
type AppManifest struct {
	...
	AppRepo AppManifest_AppRepo `json:"appRepo"`
	...
	ComponentSpec []AppManifest_ComponentSpec `json:"componentSpec"`
...
// AppManifest_AppRepo Repository where Application Provider stores the application image
type AppManifest_AppRepo struct {
...
// AppManifest_ComponentSpec_NetworkInterfaces defines model for AppManifest.ComponentSpec.NetworkInterfaces.
type AppManifest_ComponentSpec_NetworkInterfaces struct {
...
// AppManifest_ComponentSpec defines model for AppManifest.ComponentSpec.
type AppManifest_ComponentSpec struct {
	...
	NetworkInterfaces []AppManifest_ComponentSpec_NetworkInterfaces `json:"networkInterfaces"`
}
...
// ClusterInfo Kubernetes cluster information
type ClusterInfo struct {
	// ClusterRef A global unique identifier associated with a Kubernetes cluster
	// infrastructure.
	ClusterRef KubernetesClusterRef `json:"clusterRef"`

	// EdgeCloudZoneId Unique identifier created by the Edge Cloud Platform to identify an
	// Edge Cloud Zone within an Edge Cloud.
	EdgeCloudZoneId EdgeCloudZoneId `json:"edgeCloudZoneId"`

	// Name Name of the Cluster, scoped to the Provider
	Name string `json:"name"`
...
type K8sNetworking struct {
	...
	AdditionalNetworks *[]K8sNetworking_AdditionalNetworks `json:"additionalNetworks,omitempty"`

	// PrimaryNetwork Definition of Kubernetes primary Network
	PrimaryNetwork *K8sNetworking_PrimaryNetwork `json:"primaryNetwork,omitempty"`
}
...
// K8sNetworking_AdditionalNetworks Additional network interface definition
type K8sNetworking_AdditionalNetworks struct {
...
// K8sNetworking_PrimaryNetwork Definition of Kubernetes primary Network
type K8sNetworking_PrimaryNetwork struct {
...
type KubernetesResources struct {
	...
	ApplicationResources KubernetesResources_ApplicationResources `json:"applicationResources"`
...
// KubernetesResources_ApplicationResources_CpuPool_Topology CPU pool topology defines an application's CPU-based
// architecture.
...
// requirements.
type KubernetesResources_ApplicationResources_CpuPool_Topology struct {
...
// KubernetesResources_ApplicationResources_CpuPool CPU Pool refers to the amount of application' resources
...
type KubernetesResources_ApplicationResources_CpuPool struct {
	...
	// and minimum CPU/memory resource per Kubernetes node
	// requirements.
	Topology KubernetesResources_ApplicationResources_CpuPool_Topology `json:"topology"`
}

// KubernetesResources_ApplicationResources_GpuPool_Topology GPU pool topology defines an application's GPU-based
// architecture.
// When deploying for high availability or redundancy, it
// allows for clustering with a configurable number of nodes
// and minimum CPU/memory/GPU memory resource per Kubernetes
// node requirements.
type KubernetesResources_ApplicationResources_GpuPool_Topology struct {
	// MinNodeCpu Minimum number of vcpus in whole (i.e 1) per cluster
	// node in GPU pool.
	MinNodeCpu int `json:"minNodeCpu"`
...
// KubernetesResources_ApplicationResources_GpuPool GPU Pool refers to the amount of resources of the application
...
type KubernetesResources_ApplicationResources_GpuPool struct {
	...
	// and minimum CPU/memory/GPU memory resource per Kubernetes
	// node requirements.
	Topology KubernetesResources_ApplicationResources_GpuPool_Topology `json:"topology"`
}

// KubernetesResources_ApplicationResources Application resources define the resources pool required
// by the application to be executed in a Kubernetes clusters.
type KubernetesResources_ApplicationResources struct {
	...
	// exclusively in a GPU pool.
	// A CPU pool is composed by CPU and memory.
	CpuPool *KubernetesResources_ApplicationResources_CpuPool `json:"cpuPool,omitempty"`
	...
	// exclusively in a CPU pool.
	// A GPU pool is composed by memory, CPU and GPU memory
	GpuPool *KubernetesResources_ApplicationResources_GpuPool `json:"gpuPool,omitempty"`
}
...
type GetClustersParams struct {
	...
	ClusterRef *KubernetesClusterRef `form:"clusterRef,omitempty" json:"clusterRef,omitempty"`
...
func NewGetClustersRequest(server string, params *GetClustersParams) (*http.Request, error) {
	...
	if params != nil {
		...
		if params.ClusterRef != nil {

			if queryFrag, err := runtime.StyleParamWithLocation("form", true, "clusterRef", runtime.ParamLocationQuery, *params.ClusterRef); err != nil {
				return nil, err
			} else if parsed, err := url.ParseQuery(queryFrag); err != nil {
				return nil, err
			} else {
...
// GetBody returns the HTTP response's body
func (r GetAppInstanceResponse) GetBody() []byte {
	return r.Body
}
...
// GetBody returns the HTTP response's body
func (r CreateAppInstanceResponse) GetBody() []byte {
	return r.Body
}
...
// GetBody returns the HTTP response's body
func (r DeleteAppInstanceResponse) GetBody() []byte {
	return r.Body
}
...
// GetBody returns the HTTP response's body
func (r SubmitAppResponse) GetBody() []byte {
	return r.Body
}
...
// GetBody returns the HTTP response's body
func (r GetEdgeCloudZonesResponse) GetBody() []byte {
	return r.Body
}
...
type ServerInterface interface {
	...
	// Retrieve a list of the available clusters
	// (GET /clusters)
...
func (w *ServerInterfaceWrapper) GetClusters(ctx echo.Context) error {
	...
	ctx.Set(OpenIdScopes, []string{"edge-application-management:clusters:read"})
	...
	// ------------- Optional query parameter "clusterRef" -------------

	err = runtime.BindQueryParameter("form", true, false, "clusterRef", ctx.QueryParams(), &params.ClusterRef)
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, fmt.Sprintf("Invalid format for parameter clusterRef: %s", err))
	}
...
type GetClusters200ResponseHeaders struct {
	XCorrelator openapi_types.UUID
}

type GetClusters200JSONResponse struct {
	Body    []ClusterInfo
	Headers GetClusters200ResponseHeaders
}
...
type GetClusters401JSONResponse struct{ N401JSONResponse }

func (response GetClusters401JSONResponse) VisitGetClustersResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("x-correlator", fmt.Sprint(response.Headers.XCorrelator))
	w.WriteHeader(401)
...
type GetClusters403JSONResponse struct{ N403JSONResponse }

func (response GetClusters403JSONResponse) VisitGetClustersResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("x-correlator", fmt.Sprint(response.Headers.XCorrelator))
	w.WriteHeader(403)
...
type GetClusters404JSONResponse struct{ N404JSONResponse }

func (response GetClusters404JSONResponse) VisitGetClustersResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("x-correlator", fmt.Sprint(response.Headers.XCorrelator))
	w.WriteHeader(404)
...
type GetClusters500JSONResponse struct{ N500JSONResponse }
...
type GetClusters503JSONResponse struct{ N503JSONResponse }

func (response GetClusters503JSONResponse) VisitGetClustersResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("x-correlator", fmt.Sprint(response.Headers.XCorrelator))
	w.WriteHeader(503)
...
func (sh *strictHandler) GetClusters(ctx echo.Context, params GetClustersParams) error {
